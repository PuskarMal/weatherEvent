...
function fetchWeather(city1) {
  ...
  <div style="margin-top: 12px; font-size: 1rem; ">
    ...
    <p style="margin: 4px 0;">🙂 Feels like ${feels_like}°C </p>
      ...
      fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=temperature_2m_max,relative_humidity_2m_max,rain_sum,precipitation_probability_mean,uv_index_max&timezone=auto`)
  .then(res => res.json())
  .then(meteo => {
    const { time, temperature_2m_max, relative_humidity_2m_max, precipitation_probability_mean, rain_sum, uv_index_max } = meteo.daily;
    ...
    for (let i = 0; i < time.length && daysDisplayed < 5; i++) {
      ...
      const rainAmt = Math.round(rain_sum[i]);
      const precAmt = Math.round(precipitation_probability_mean[i])
      const uv = Math.round(uv_index_max[i]);
      
      feedContainer.innerHTML += `
        <div style='display:flex; gap:12px; margin-bottom:10px; border-bottom:1px solid #ccc; padding-bottom:6px;'>
          <div><strong>${dayName}</strong></div>
          <div title='Temperature'>🌡️ ${temp}°C</div>
          <div title='Humidity'>💧 ${humidity}%</div>
          <div title='Total Rain'>🌧️ ${rainAmt} mm</div>
          <div title='Precipitation'>❄️ ${precAmt} mm</div>
          <div title='UV Index'>☀️ UV: ${uv}</div>
        </div>
      `;
      daysDisplayed++;
    }
  ...
  .catch(err => {
    document.getElementById("historyFeed").innerHTML = `<span style="color:red;">${err.message}</span>`;
  });
